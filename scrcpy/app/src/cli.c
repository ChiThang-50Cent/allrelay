...
static const struct sc_option options[] = {
    ...
    {
        .longopt_id = OPT_WIFI_MODE,
        .longopt = "wifi",
        .text = "Connect directly to phone via Wi-Fi (AllRelay mode). "
                "Bypasses ADB tunnel. Requires --tunnel-host to be set.",
    },
    {
        .longopt_id = OPT_WIFI_PORT,
        .longopt = "wifi-port",
        .argdesc = "port",
...
parse_args_with_getopt(struct scrcpy_cli_args *args, int argc, char *argv[],
    ...
    while ((c = getopt_long(argc, argv, optstring, longopts, NULL)) != -1) {
        switch (c) {
            ...
            case OPT_WIFI_MODE:
                ...
                break;
            case OPT_WIFI_PORT:
                if (!parse_port(optarg, &opts->wifi_port)) {
                    return false;
                }
                break;
    ...
    if (opts->wifi_mode) {
        if (!opts->tunnel_host) {
            LOGE("--wifi requires --tunnel-host to be set (phone IP address)");
            return false;
        }
        if (!opts->wifi_port) {
            opts->wifi_port = 5000; // default AllRelay port
        }

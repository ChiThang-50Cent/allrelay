...
sc_server_connect_wifi(struct sc_server *server, struct sc_server_info *info) {
    ...
    uint32_t host = server->params.tunnel_host;
    if (!host) {
        LOGE("Wi-Fi mode requires --tunnel-host to be set");
        goto fail;
    }

    uint16_t base_port = server->params.wifi_port;
    if (!base_port) {
        base_port = 5000; // default AllRelay port
    }

    unsigned attempts = 100;
    sc_tick delay = SC_TICK_FROM_MS(200);

    LOGI("Connecting to %08x:%u (Wi-Fi mode)", host, base_port);

    // Connect to video port
    if (video) {
        LOGD("Connecting to video port %u", base_port);
        server->video_socket = connect_to_server(server, attempts, delay,
                                                 host, base_port);
        if (server->video_socket == SC_SOCKET_NONE) {
            LOGE("Could not connect to video port");
            goto fail;
        }
    }
    ...
    // Connect to control port (base_port + 4)
    if (control) {
        uint16_t control_port = base_port + 4;
        LOGD("Connecting to control port %u", control_port);
        server->control_socket = connect_to_server(server, attempts, delay,
                                                   host, control_port);
        if (server->control_socket == SC_SOCKET_NONE) {
            LOGE("Could not connect to control port");
            goto fail;
        }
    }
    ...
    LOGI("Wi-Fi connected to %s", info->device_name);
    return true;
...
run_server(void *data) {
    ...
    // AllRelay Wi-Fi mode: connect directly to phone, skip ADB entirely
    // Server must already be running on the phone in Wi-Fi mode
    if (params->wifi_mode) {
        LOGI("Wi-Fi mode: connecting directly to phone (skipping ADB push)");
        bool ok = sc_server_connect_wifi(server, &server->info);
        if (!ok) {
            goto error_connection_failed;
        }

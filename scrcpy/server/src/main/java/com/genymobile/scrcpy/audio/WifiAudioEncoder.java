...
/**
 * Wi-Fi aware AudioEncoder that writes to WifiStreamer instead of Streamer.
 *
 * This class handles audio capture and Opus encoding for direct Wi-Fi transport.
 * It is the audio equivalent of WifiSurfaceEncoder.
 */
public final class WifiAudioEncoder implements AsyncProcessor {

    private static class InputTask {
        private final int index;
        InputTask(int index) { this.index = index; }
    }
    ...
    public WifiAudioEncoder(AudioCapture capture, WifiStreamer streamer, Options options) {
    ...
    private void inputThread(MediaCodec mediaCodec, AudioCapture capture) throws IOException, InterruptedException {
        ...
        while (!Thread.currentThread().isInterrupted()) {
            ...
            mediaCodec.queueInputBuffer(task.index, bufferInfo.offset, bufferInfo.size,
    ...
    public void start(TerminationListener listener) {
        thread = new Thread(() -> {
            ...
            try {
                ...
                // expected when audio not available
            } catch (IOException e) {
                Ln.e("Audio encoding error", e);
                fatalError = true;
            } finally {
                Ln.d("Wi-Fi audio encoder stopped");
    ...
    private void encode() throws IOException, ConfigurationException, AudioCaptureException {
        ...
        try {
            ...
            try {
                if (mediaCodecThread != null) mediaCodecThread.join();
            ...
            if (mediaCodec != null) {
                if (mediaCodecStarted) mediaCodec.stop();
                mediaCodec.release();
            }
            if (capture != null) {
                capture.stop();
        ...
        if (encoderName != null) {
            ...
            try {
                ...
                if (!codec.getMimeType().equals(mimeType)) {
                    Ln.e("Audio encoder type for \"" + encoderName + "\" (" + mimeType
                            + ") does not match codec type (" + codec.getMimeType() + ")");

#!/bin/bash
# allrelay-keep-alive.sh — Keep AllRelay Android server running
# Restarts server when it stops listening on ports
...
check_ports() {
    ...
    local ports=$(adb shell "ss -tlnp | grep -c 500" 2>/dev/null | tr -d ' \r\n')
    if [ "$ports" -ge 4 ]; then
        return 0
    else
        return 1
...
monitor_loop() {
    ...
    while true; do
        if ! check_ports; then
            log "Ports not listening, restarting server..."
            start_server
        fi
...
# Main
log "=== AllRelay Keep-Alive Starting ==="

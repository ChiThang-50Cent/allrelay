#!/bin/bash
# monitor.sh — Start AllRelay monitor (screen mirror + input) on Ubuntu
# Usage: ./monitor.sh [--host IP]
set -e
...
echo "=== AllRelay Monitor (Screen + Input) ==="
echo "Host: $HOST"
...
adb shell "su -c 'CLASSPATH=$SRV app_process / com.genymobile.scrcpy.Server 4.0 log_level=info max_size=2640 wifi_mode=true wifi_port=5000 max_fps=60 video_codec=h264 audio_source=mic multistream=true >> /data/allrelay/logs/allrelay.log 2>&1 &'"
...
# Step 3: Start Go relay (screen + input only)
echo "[3/4] Starting AllRelay monitor..."
echo "  Screen → GStreamer window (click on it to capture input)"
echo "  Input  → Keyboard/Mouse forwarded to phone"
"$SERVER_BIN" \
    --host "$HOST" \
    --no-camera --no-mic --no-speaker --no-heartbeat \
    2>&1 &
SERVER_PID=$!
sleep 3

if kill -0 $SERVER_PID 2>/dev/null; then
    echo "  Monitor running (PID $SERVER_PID)"
else
...
echo " Click inside the window to capture input"
echo " Keyboard/mouse → forwarded to phone"
echo "============================================"
echo ""
